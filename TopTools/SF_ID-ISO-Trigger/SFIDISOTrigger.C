...
void SFIDISOTrigger(std::vector<float> &result,
  ...
  ***************************/
  if(channel == 0){
    ...
    int NbinmupT =  hmuIDISOSF->GetNbinsX();  // Y-axis (pT)
    ...
    int NbinTrmupT  = hmuTriggerSF->GetNbinsX(); // Y-axis (pT)
  ...
  ***************************/
  if(channel == 1){
    ...
    int NbinepT  = heIDISOSF->GetNbinsX();  // Y-axis (pT)
    ...
    int NbinTrepT  = heTriggerSF->GetNbinsX(); // Y-axis (pT)
